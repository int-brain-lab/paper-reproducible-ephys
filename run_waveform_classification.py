...
features_1d_PPC = waveforms_df[waveforms_df['regions'] == 'PPC'][FEATURES_1D].to_numpy()
features_2d_PPC = waveforms_df[waveforms_df['regions'] == 'PPC'][FEATURES_2D].to_numpy()
kmeans = KMeans(n_clusters=3, random_state=0).fit(features_1d_PPC)
km_ids_1d = kmeans.labels_
kmeans = KMeans(n_clusters=3, random_state=0).fit(features_2d_PPC)
km_ids_2d = kmeans.labels_
waveforms_df.loc[waveforms_df['regions'] == 'PPC', 'group'] = km_ids_2d

# Do TNSE embedding
tsne_ppc_1d = TSNE(n_components=2, perplexity=30, random_state=55).fit_transform(features_1d_PPC)
tsne_ppc_2d = TSNE(n_components=2, perplexity=30, random_state=55).fit_transform(features_2d_PPC)

f, (ax1, ax2) = plt.subplots(1, 2, figsize=(10, 5), dpi=150)

ax1.scatter(tsne_ppc_2d[km_ids_1d == 0, 0], tsne_ppc_1d[km_ids_1d == 0, 1])
ax1.scatter(tsne_ppc_2d[km_ids_1d == 1, 0], tsne_ppc_1d[km_ids_1d == 1, 1])
ax1.scatter(tsne_ppc_2d[km_ids_1d == 2, 0], tsne_ppc_1d[km_ids_1d == 2, 1])
ax1.get_xaxis().set_visible(False)
ax1.get_yaxis().set_visible(False)
ax1.set(title='1D waveform features')

ax2.scatter(tsne_ppc_2d[km_ids_2d == 0, 0], tsne_ppc_2d[km_ids_2d == 0, 1], label='RS2')
ax2.scatter(tsne_ppc_2d[km_ids_2d == 1, 0], tsne_ppc_2d[km_ids_2d == 1, 1], label='RS1')
ax2.scatter(tsne_ppc_2d[km_ids_2d == 2, 0], tsne_ppc_2d[km_ids_2d == 2, 1], label='FS')
ax2.get_xaxis().set_visible(False)
ax2.get_yaxis().set_visible(False)
ax2.set(title='1D + 2D waveform features')
ax2.legend(frameon=False)

plt.savefig(join(FIG_PATH, 't-SNE_embedding_PPC'))

# %% Plot average spike from clusters
...
for i in waveforms_df.loc[waveforms_df['group'] == 0].index:
...
for i in waveforms_df.loc[waveforms_df['group'] == 1].index:
...
for i in waveforms_df.loc[waveforms_df['group'] == 2].index:
...
plt.savefig(join(FIG_PATH, '2D_waveform_groups'))
