...
for i, region in enumerate(REGIONS):
    ...
    kmeans = KMeans(n_clusters=N_TYPES[region], random_state=42).fit(features_2d)
    waveforms_df.loc[waveforms_df['regions'] == region, 'group_label'] = kmeans.labels_

    # Get the RS and FS labels right
    fs_label = waveforms_df[waveforms_df['regions'] == region].groupby('group_label').mean()['firing_rate'].idxmax()
    waveforms_df.loc[(waveforms_df['regions'] == region) & (waveforms_df['group_label'] == fs_label), 'type'] = 'FS'

    if N_TYPES[region] == 2:
        rs_label = waveforms_df[waveforms_df['regions'] == region].groupby('group_label').mean()['firing_rate'].idxmin()
        waveforms_df.loc[(waveforms_df['regions'] == region) & (waveforms_df['group_label'] == rs_label), 'type'] = 'RS'
        ...
        rs2_label = waveforms_df[waveforms_df['regions'] == region].groupby('group_label').mean()['v_below'].idxmin()
        types = np.array([0, 1, 2])
        rs1_label = types[~np.isin(types, np.array([fs_label, rs2_label]))]

        waveforms_df.loc[(waveforms_df['regions'] == region) & (waveforms_df['group_label'] == 1), 'type'] = 'RS1'
        waveforms_df.loc[(waveforms_df['regions'] == region) & (waveforms_df['group_label'] == 1), 'type'] = 'RS2'
