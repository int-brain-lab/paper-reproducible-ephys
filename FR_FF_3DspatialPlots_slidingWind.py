...
SaveStr = '_SlidingWindow' #string to have at the end of the figure/file names to be saved, e.g., the date
   ...
   # mask = np.logical_and(clusters['acronym'] == 'LP', clusters['metrics']['label'] == 1)
    ...
    for k, cluster in enumerate(clusterIDs):
        ...
        ActivitySmallBin = [cluster_peths_FR_FF_sliding(spikes['times'][spikes['clusters'] == cluster], 
                                                      ...
                                                      hist_win=binSzFRPeri, N_SlidesPerWind = n_slideFR) for x in range(0,len(event_times))]
        ...
        ActivitySlideLargeBin = [cluster_peths_FR_FF_sliding(spikes['times'][spikes['clusters'] == cluster], 
                                                      event_times[x], pre_time=pre_time, post_time=post_time,
                                                      hist_win=binSzFFPeri, N_SlidesPerWind = n_slideFF) for x in range(0,len(event_times))]
    ...
    columnsPeri=['eID', 'probeNum', 'dateInfo', 'Subject', 'LabID','clusterID', 
                 ...
                 'TimeVect_FR', 'TimeVect_FF', 'FRoverT', 'FR_STD_overT', 'FFoverT', 'event_Titles',
                 'RepeatedSite', 'SpikeSortMethod']
    if len(clusterIDs)>1:
        ...
        dataPeri = np.array([np.repeat(eid, len(clusterIDs)), np.repeat(probe, len(clusterIDs)), 
                             ...
                             event_TitlesAll, np.repeat(RS_YorN, len(clusterIDs)), SpikeSortMethod]) #np.array(FRoverT_ev0).reshape(76,44) for 1st eid
        ...
        dataPeri = np.array([eid, probe, DateInfo, subj, np.squeeze(LabNumArray), np.squeeze(np.array(clusterIDs)), np.squeeze(np.array(Ntrials)),
                             ...
                             event_TitlesAll, RS_YorN, np.squeeze(SpikeSortMethod)])
...
if SaveFigs==1:
    # If needed, use codes below to save DataFrame and save as .mat file for Matlab
    ClusterFeatures.to_pickle(FIG_PATH+'/'+regions+'/'+regions +"_ClusterFeatures_DF_" + SaveStr + ".pkl")
    PeriEventFRandFF.to_pickle(FIG_PATH+'/'+regions+'/'+regions +"_PeriEventFRandFF_DF_" + SaveStr + ".pkl")
    # output = pd.read_pickle("ClusterFeatures_DF.pkl")
    # print(output)
    sio.savemat(FIG_PATH+'/'+regions+'/'+regions +"_ClusterFeatMat_" + SaveStr + ".mat", {name: col.values for name, col in ClusterFeatures.items()})
    sio.savemat(FIG_PATH+'/'+regions+'/'+regions +"_PeriEventFRandFF_" + SaveStr + ".mat", {name: col.values for name, col in PeriEventFRandFF.items()})
