...
Gather PROBE displacement data.
...
micro manipulator, planned - the first is always returned, UNLESS specified by
the provenance= ARG.
...
def download_probe_disp_data(x, y, 
                             project='ibl_neuropixel_brainwide_01'):
    """Download probe displacement data for a given probe at [x,y] from Alyx
    ...
    * mouse_recording_weight dob
    ...
    save_probe_disp_data(data_frame, prefix )
    
    return data_frame



def download_probe_disp_data_rep_site():
    ...
    * hist_to_micro_error_surf hist_to_micro_error_tip
    ...
    return download_probe_disp_data(-2243, -2000)



def save_probe_disp_data(data_frame, prefix):
...
def load_probe_disp_data(prefix, suffix='_probe_disp.csv'):
...
def load_probe_disp_data_rep_site():
    ...
    """
    return load_probe_disp_data("-2243_-2000")



# if file is executed as main - fetch repeated site data and save cache as CSV
if __name__ == "__main__":
    data_frame = download_probe_disp_data_rep_site()
...
def old_fetch_rep_site_ch_disp_from_planned_data():
    """Fetch repeated site channels displacement data from Alyx
    
    Fetches the most up-to-date data from Alyx for repeated site recordings,
    their channels, and the orthogonal points of each channel from the planned
    trajectory.
    
    Saves this data to a standard location in the file system.
    
    Returns this data as a pandas DataFrame object with following:
    ...
    * dist - the 3D distance between chan_loc xyz and planned_orth_proj xyz
    ...
    old_save_rep_site_ch_disp_from_planned_data(data_frame)
...
def old_save_rep_site_ch_disp_from_planned_data(data_frame):
    """Save repeated site channels displacement data
    
    data_frame should be a pandas DataFrame object
    ...
    PROBE_DATA_REL_PATH = Path('histology', 
                               'probe_data', 
                               'rep_site_ch_disp_from_planned.csv')
