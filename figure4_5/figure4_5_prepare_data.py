...
def prepare_data(insertions, one, recompute=False, **kwargs):
    ...
    save_path = save_data_path(figure='figure4_5')
    print(f'Saving data to {save_path}')
    concat_df.to_csv(save_path.joinpath('figure4_5_dataframe.csv'))
    smoothing = smoothing or 'none'
    norm = norm or 'none'
    np.savez(save_path.joinpath(f'figure4_5_data_event_{align_event}_smoothing_{smoothing}_norm_{norm}.npz'), **data)
