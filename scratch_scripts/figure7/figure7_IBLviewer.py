...
from figure7.figure7_load_data import load_dataframe
...
if __name__ == '__main__':
    ...
    #AvgFR = df_reg['avg_fr']
    AvgFR = np.log10(df_reg['avg_fr']) #log10 of the firing rate
    

    PtRadius = 7 #12
    PtNoise = 9 #10 

    # Extract xyz coordinates and the value 
    ValuesPerKey = np.array(AvgFR).astype(float) #convert array of object to array of float64.  ##peak-trough, amps, AvgFF
    XYZs = np.vstack([np.array(df_reg['x']), np.array(df_reg['y']), np.array(df_reg['z'])]).transpose().astype(float)*1e6 #convert array of object to array of float64


    # Make colormap match Matlab's Parula:
