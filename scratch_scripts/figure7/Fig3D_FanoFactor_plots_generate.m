...
pMWU_corrFR_all = nan(5, 5); %5 brain regions possible & 5features beinng compared (x,y,z,amp,dur)
f = figure(1);
...
for regionN = 1:size(BrainRegions,2)
    ...
    for spHist=1:3
        subplot(3,1,spHist)
        Sample1(:,spHist) = dXYZ(UnitsHighFF, spHist);
        Sample2(:,spHist) = dXYZ(UnitsLowFF, spHist);
    ...
    for spHist=4:5
        ...
        Sample1(:,spHist) = SpikeWF(UnitsHighFF, spHist-3);
        Sample2(:,spHist) = SpikeWF(UnitsLowFF, spHist-3);
